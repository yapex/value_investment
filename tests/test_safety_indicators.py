"""Test Safety Indicators - CashToDebtIndicator and DebtRatioTotalIndicator"""
import pytest
import pandas as pd
...
def test_cash_to_debt_indicator():
    ...
    balance = pd.DataFrame({
        ...
        'MONETARYFUNDS': [1000.0, 1000.0],  # 货币资金
        'SHORT_LOAN': [200.0, 200.0],      # 短期借款
        'LONG_LOAN': [300.0, 300.0],       # 长期借款
    ...
    # cash_to_debt = 1000 / 600 = 1.67
    assert result.value == pytest.approx(1.67, 0.01)
...
def test_cash_to_debt_no_debt():
    ...
    # When debt is 0, should return a high value (or infinity handling)
...
def test_debt_ratio_total_indicator():
    ...
    # 有息负债 = 600, 总资产 = 5000
    # debt_ratio_total = 600 / 5000 * 100 = 12%
    assert result.value == pytest.approx(12.0, 0.1)
    assert result.unit == "%"
    assert "Debt Ratio Total" in result.description or "有息负债/总资产" in result.description
...
def test_cash_to_debt_required_fields():
    """CashToDebtIndicator should report required fields"""
    indicator = CashToDebtIndicator()
    fields = indicator.get_required_fields()

    assert 'MONETARYFUNDS' in fields
    assert 'SHORT_LOAN' in fields
    assert 'LONG_LOAN' in fields


def test_debt_ratio_total_required_fields():
    """DebtRatioTotalIndicator should report required fields"""
    indicator = DebtRatioTotalIndicator()
    fields = indicator.get_required_fields()
