"""美股财务报表 Handler (资产负债表 + 利润表 + 现金流量表)"""
...
# 美股财务报表字段（来自 CORE_FIELD_MAPPING 中 美股 的映射）
US_STOCK_STATEMENT_FIELDS: set[str] = {
    # 利润表
    "total_revenue",
    "net_profit",
    "operating_profit",
...
class USStockStatementHandler(BaseHandler):
    """美股财务三表 Handler
    ...
    def __init__(self, provider=None):
        super().__init__(provider, "美股", US_STOCK_STATEMENT_FIELDS)

    async def _handle_impl(self, message: "Message") -> None:
        """处理美股财务报表请求

        TODO: 待 US Provider 实现 fetch_financial_data 后完善此方法
        """
